...
for iter = 1:parameters.iteration_count
    ...
    if 0
        ...
        q = zeros(P, 1);
        for m = 1:P
            q(m) = trace(Km(:, :, m)) - trace(H' * Km(:, :, m) * H);
        end
        theta = lp_weight_proj(q, 2);
    end
    K_theta = calculate_kernel_theta(Km, theta.^2);
...
reset(stream);
if ~isempty(which('litekmeans'))
    state.clustering = litekmeans(H_normalized, parameters.cluster_count, 'maxIter', 1000, 'Replicates', 10);
else
