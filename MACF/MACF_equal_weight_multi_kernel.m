function MACF_equal_weight_multi_kernel(dataset, kernel_type, nRepeat)
...
disp(['MACF on equal weighted multi kernel begin ...']);
ew_macf_res_file = fullfile(res_dir, [dataset, '_res_macf_ew.mat']);
if ~exist(ew_macf_res_file, 'file')
    t_start = clock;
    ew_macf_res = MACF_single_kernel(Ka, y, fullfile(res_dir, [dataset, '_ewmk']), nRepeat);
    t_end = clock;
    disp(['MACF exe time: ', num2str(etime(t_end, t_start))]);
    save(ew_macf_res_file, 'ew_macf_res');
else
    load(ew_macf_res_file, 'ew_macf_res');
end
disp(['MACF on on equal weighted multi kernel done']);
if size(ew_macf_res, 1) > 1
	res_ew_macf_aio = mean(ew_macf_res);
else
    res_ew_macf_aio = ew_macf_res;
end
clear K ew_macf_res;

save(fullfile(res_dir, [dataset, '_res_macf_ew_kernel.mat']), 'res_ew_macf_aio', 'kernel_list');
end