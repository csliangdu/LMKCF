function KCF_orth_all_kernel(dataset, kernel_type, nRepeat)
...
for iKernel = 1:length(kernel_list)
	...
	disp(['KCF orth on ',  num2str(iKernel), ' of ' num2str(length(kernel_list)), ' Kernel(',  iFile(1:end-4), ') begin ...']);
    kcf_res_file = fullfile(res_dir, [iFile(1:end-4), '_res_kcf_orth.mat']);
    if ~exist(kcf_res_file, 'file')
        t_start = clock;
        kcf_res =KCF_orth_single_kernel(K, y, fullfile(res_dir, iFile(1:end-4)), nRepeat);
        t_end = clock;
	...
	if size(kcf_res, 1) > 1
		res_kcf_orth_aio = [res_kcf_orth_aio; mean(kcf_res)]; %#ok<AGROW>
	else
		res_kcf_orth_aio = [res_kcf_orth_aio; kcf_res]; %#ok<AGROW>
	end
    clear K kcf_res;
end

save(fullfile(res_dir, [dataset, '_res_kcf_orth_all_kernel.mat']), 'res_kcf_orth_aio', 'kernel_list');
end