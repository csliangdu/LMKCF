...
% kcf_algo = 'kcf';
% if isfield(options,'kcf_algo')
%     kcf_algo = options.kcf_algo;
% end
...
while tryNo < nRepeat
    ...
    while(maxErr > differror)
        ...
        [U,V] = NormalizeUV(K, U, V, NormV, Norm);
        %         switch lower(kcf_algo)
        %             case 'kcf'
        %                 [U,V] = NormalizeUV(K, U, V, NormV, Norm);
        %                 [U, V] = KCF_Multi(K, c, struct('maxIter', [], 'nRepeat', 1), U, V);
        %                 [U,V] = NormalizeUV(K, U, V, NormV, Norm);
        %             case 'kcf_orth'
        %                 [U, V] = KCF_orth_Multi(K, c, struct('maxIter', 100, 'nRepeat', 1, 'lambda', lambda), U, V);
        %             case 'kcf_simplex'
        %                 % [U, V] = KCF_simplex_Multi(K, c, struct('maxIter', 100, 'nRepeat', 1, 'lambda', lambda), U, V);
