function LCCF_equal_weight_multi_kernel(dataset, kernel_type, nRepeat)
...
disp(['LCCF on equal weighted multi kernel begin ...']);
ew_lccf_res_file = fullfile(res_dir, [dataset, '_res_lccf_ew.mat']);
if ~exist(ew_lccf_res_file, 'file')
    t_start = clock;
    ew_lccf_res = LCCF_single_kernel(Ka, y, fullfile(res_dir, [dataset, '_ewmk']), nRepeat);
    t_end = clock;
    disp(['LCCF exe time: ', num2str(etime(t_end, t_start))]);
    save(ew_lccf_res_file, 'ew_lccf_res');
else
    load(ew_lccf_res_file, 'ew_lccf_res');
end
disp(['LCCF on on equal weighted multi kernel done']);
if size(ew_lccf_res, 1) > 1
	res_ew_lccf_aio = mean(ew_lccf_res);
else
    res_ew_lccf_aio = ew_lccf_res;
end
clear K ew_lccf_res;

save(fullfile(res_dir, [dataset, '_res_lccf_ew_kernel.mat']), 'res_ew_lccf_aio', 'kernel_list');
end